...
import { englishRoadmap } from '../roadmap/english-roadmap';
...
interface PracticeVolume {
  ...
  error_id_exercises?: number;
  sentence_transformation?: number;
  full_reading_tests?: number;
}
...
interface EnglishRoadmapData {
  subject: string;
  exam: {
    total_questions: number;
    time_minutes: number;
    target_score: number;
    structure?: Array<{ part: string; questions: number; skills: string[] }>;
  };
  timeline: TimelineItem[];
  grammar_summary: GrammarSummary;
  common_mistakes: string[];
  study_tips: string[];
  vocabulary_themes?: Array<{ theme: string; key_words: string[] }>;
}

const data = englishRoadmap as EnglishRoadmapData;

// --- Components ---

const StatusBadge = ({ phase }: { phase: string }) => {
  const getColor = (p: string) => {
    if (p.includes('Foundation')) return 'bg-emerald-100 text-emerald-700 border-emerald-200';
...
const ChecklistItem = ({ 
  ...
  return (
    <div 
      ...
      onClick={() => onToggle(id)}
    >
      <div className={`mt-0.5 shrink-0 transition-colors ${checked ? 'text-emerald-600' : 'text-slate-300 group-hover:text-emerald-400'}`}>
...
export function EnglishRoadmap() {
  ...
  const [activeGrammarTab, setActiveGrammarTab] = useState<string>('thi');
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('english-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('english-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
  ...
  const grammarCategories = {
    thi: "Các thì",
    cau_bi_dong: "Câu bị động",
    cau_dieu_kien: "Câu điều kiện",
    menh_de_quan_he: "Mệnh đề quan hệ",
    trong_am: "Trọng âm"
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-slate-500 text-sm font-medium mb-1">
                ...
                <span className="text-emerald-600 font-bold">Môn {data.subject}</span>
              </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between mb-2">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-emerald-600" /> Timeline Chi Tiết
                </h2>
                ...
                return (
                  <div 
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                        isExpanded 
                        ? 'border-emerald-500 ring-4 ring-emerald-500/10 shadow-lg relative z-10' 
                    ...
                    >
                        ...
                        <div className={`relative flex flex-col items-center justify-center w-16 h-16 rounded-xl border shrink-0 transition-colors ${
                            ...
                            {isComplete ? (
                              ...
                              <>
                                <span className="text-xs font-bold uppercase tracking-wider">{item.month.substring(0, 5)}</span>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 font-medium">
                                ...
                                <span>{moduleCount} chủ đề</span>
                            </div>
                            
                            {/* Linear Progress Bar for Header */}
                            {progress > 0 && (
                              <div className="mt-3 max-w-[200px]">
                                <ProgressBar value={progress} max={100} colorClass={progress === 100 ? "bg-green-500" : "bg-emerald-500"} />
                              </div>
                            )}
                        </div>
                                      ...
                                      {item.modules.map((mod, mIdx) => (
                                          <div key={mIdx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                              <div className="font-bold text-slate-900 mb-2 pb-2 border-b border-slate-100 flex items-center gap-2">
                                                  <Book className="w-4 h-4 text-emerald-500" /> {mod.name}
                                              </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
                                        ...
                                        {item.practice_volume.vocabulary_words && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Từ vựng</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.vocabulary_words} từ</span>
                                                </div>
                                        ...
                                        {item.practice_volume.cloze_tests && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Cloze Tests</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.cloze_tests} bài</span>
                                                </div>
                                                <ProgressBar value={progress} max={100} colorClass="bg-teal-500" />
                                            </div>
                                        )}
                                        {item.practice_volume.reading_passages && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Bài đọc hiểu</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.reading_passages} bài</span>
                                                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-yellow-300" /> Sổ Tay Ngữ Pháp
                    </h3>
                </div>
                
                {/* Tabs */}
                <div className="flex overflow-x-auto border-b border-slate-100 p-2 gap-1 no-scrollbar">
                    {Object.keys(grammarCategories).map((key) => (
                        <button
                            key={key}
                            onClick={() => setActiveGrammarTab(key)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${
                                activeGrammarTab === key 
                                ? 'bg-emerald-100 text-emerald-700' 
                                : 'bg-transparent text-slate-500 hover:bg-slate-50'
                            }`}
                        >
                            {/* @ts-ignore */}
                            {grammarCategories[key]}
                        </button>
                    ))}
                </div>
                ...
                <div className="p-4 max-h-[300px] overflow-y-auto bg-slate-50/50">
                     <ul className="space-y-3">
                        {/* @ts-ignore */}
                        {data.grammar_summary[activeGrammarTab]?.map((formula, idx) => (
                ...
                <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
                    <button className="text-xs font-bold text-emerald-600 hover:underline flex items-center justify-center gap-1 w-full">
                        Xem toàn bộ ngữ pháp <ArrowRight className="w-3 h-3" />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-100 p-6">
                <h3 className="font-bold text-yellow-800 mb-4 flex items-center gap-2">
                    <Lightbulb className="w-5 h-5" /> Mẹo Ôn Thi Tiếng Anh
                </h3>
            ...
            {/* 4. Vocabulary Themes */}
            {data.vocabulary_themes && (
              <div className="bg-blue-50 rounded-2xl border border-blue-100 p-6">
                  <h3 className="font-bold text-blue-800 mb-4 flex items-center gap-2">
                      <Languages className="w-5 h-5" /> Từ Vựng Theo Chủ Đề
                  </h3>
                  <div className="space-y-4">
                      {data.vocabulary_themes.map((theme, idx) => (
                          <div key={idx} className="bg-white/60 p-3 rounded-lg">
                              <div className="font-bold text-blue-700 text-sm mb-2">{theme.theme}</div>
                              <div className="flex flex-wrap gap-1">
                                  {theme.key_words.slice(0, 6).map((word, wIdx) => (
                                      <span key={wIdx} className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full">
                                          {word}
                                      </span>
                                  ))}
                              </div>
