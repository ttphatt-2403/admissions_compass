...
import { chemistryRoadmap } from '../roadmap/chemistry-roadmap';
...
interface PracticeVolume {
  ...
  mini_tests?: number;
  full_tests?: number;
  full_tests_organic?: number;
  full_tests_inorganic?: number;
...
interface ChemistryRoadmapData {
  subject: string;
  exam: {
...
const data = chemistryRoadmap as ChemistryRoadmapData;

// --- Components ---

const StatusBadge = ({ phase }: { phase: string }) => {
  const getColor = (p: string) => {
    if (p.includes('Foundation')) return 'bg-blue-100 text-blue-700 border-blue-200';
...
const ChecklistItem = ({ 
  ...
  return (
    <div 
      ...
      onClick={() => onToggle(id)}
    >
      <div className={`mt-0.5 shrink-0 transition-colors ${checked ? 'text-blue-600' : 'text-slate-300 group-hover:text-blue-400'}`}>
...
export function ChemistryRoadmap() {
  const [expandedMonth, setExpandedMonth] = useState<string | null>(data.timeline[0].month);
  const [activeFormulaTab, setActiveFormulaTab] = useState<string>('este_lipit');
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('chemistry-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('chemistry-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
  ...
  const formulaCategories = {
    ...
    amin_aminoaxit: "Amin - Amino Axit",
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-slate-500 text-sm font-medium mb-1">
                ...
                <span className="text-purple-600 font-bold">Môn {data.subject}</span>
              </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between mb-2">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-purple-600" /> Timeline Chi Tiết
                </h2>
                ...
                return (
                  <div 
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                        isExpanded 
                        ? 'border-purple-500 ring-4 ring-purple-500/10 shadow-lg relative z-10' 
                        : 'border-slate-200 hover:border-purple-200 hover:shadow-md'
                    } ${isComplete ? 'bg-purple-50/30' : ''}`}
                  >
                    {/* Header Card */}
                    <div 
                    ...
                    >
                        ...
                        <div className={`relative flex flex-col items-center justify-center w-16 h-16 rounded-xl border shrink-0 transition-colors ${
                            ...
                            {isComplete ? (
                              ...
                              <>
                                <span className="text-xs font-bold uppercase tracking-wider">{item.month.substring(0, 3)}</span>
                            ...
                            {!isComplete && progress > 0 && (
                              <div className="absolute -bottom-2 -right-2 bg-purple-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white">
                                {progress}%
                              </div>
                            )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 font-medium">
                                ...
                                <span>{item.modules ? item.modules.length : 0} chủ đề</span>
                            </div>
                            
                            {/* Linear Progress Bar for Header */}
                            {progress > 0 && (
                              <div className="mt-3 max-w-[200px]">
                                <ProgressBar value={progress} max={100} colorClass={progress === 100 ? "bg-green-500" : "bg-purple-500"} />
                              </div>
                            )}
                        </div>
                                    ...
                                    {item.modules.map((mod, mIdx) => (
                                        <div key={mIdx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                            <div className="font-bold text-slate-900 mb-2 pb-2 border-b border-slate-100 flex items-center gap-2">
                                                <Book className="w-4 h-4 text-purple-500" /> {mod.name}
                                            </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
                                        ...
                                        {item.practice_volume.full_tests_organic && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Full Test Hữu Cơ</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.full_tests_organic} đề</span>
                                                </div>
                                                <ProgressBar value={progress} max={100} colorClass="bg-orange-500" />
                                            </div>
                                        )}
                                        {item.practice_volume.full_tests_inorganic && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Full Test Vô Cơ</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.full_tests_inorganic} đề</span>
                                                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-slate-900 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <Calculator className="w-5 h-5 text-yellow-400" /> Sổ Tay Phản Ứng
                    </h3>
                </div>
                ...
                <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
                    <button className="text-xs font-bold text-purple-600 hover:underline flex items-center justify-center gap-1 w-full">
                        Xem toàn bộ 100+ phản ứng <ArrowRight className="w-3 h-3" />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-100 p-6">
                ...
                <ul className="space-y-4">
                    {data.study_tips.slice(0, 4).map((tip, idx) => (
