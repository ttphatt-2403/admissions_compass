...
import { gdcdRoadmap } from '../roadmap/gdcd-roadmap';
...
const data = gdcdRoadmap as GDCDRoadmapData;

// --- Components ---

const StatusBadge = ({ phase }: { phase: string }) => {
  const getColor = (p: string) => {
    if (p.includes('Foundation')) return 'bg-orange-100 text-orange-700 border-orange-200';
    if (p.includes('Advanced')) return 'bg-purple-100 text-purple-700 border-purple-200';
    if (p.includes('Intensive')) return 'bg-red-100 text-red-700 border-red-200';
    if (p.includes('Exam') || p.includes('Simulation')) return 'bg-rose-100 text-rose-700 border-rose-200';
    if (p.includes('Review') || p.includes('Final') || p.includes('Weak')) return 'bg-green-100 text-green-700 border-green-200';
...
const ChecklistItem = ({ 
  ...
  return (
    <div 
      ...
      onClick={() => onToggle(id)}
    >
      <div className={`mt-0.5 shrink-0 transition-colors ${checked ? 'text-orange-600' : 'text-slate-300 group-hover:text-orange-400'}`}>
...
export function GDCDRoadmap() {
  const [expandedMonth, setExpandedMonth] = useState<string | null>(data.timeline[0].month);
  const [activeRightsTab, setActiveRightsTab] = useState<number>(0);
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('gdcd-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('gdcd-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-slate-500 text-sm font-medium mb-1">
                ...
                <span className="text-orange-600 font-bold">Môn {data.subject}</span>
              </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between mb-2">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-orange-600" /> Timeline Chi Tiết
                </h2>
                ...
                return (
                  <div 
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                        isExpanded 
                        ? 'border-orange-500 ring-4 ring-orange-500/10 shadow-lg relative z-10' 
                        : 'border-slate-200 hover:border-orange-200 hover:shadow-md'
                    } ${isComplete ? 'bg-orange-50/30' : ''}`}
                  >
                    {/* Header Card */}
                    <div 
                    ...
                    >
                        ...
                        <div className={`relative flex flex-col items-center justify-center w-16 h-16 rounded-xl border shrink-0 transition-colors ${
                            ...
                            {!isComplete && progress > 0 && (
                              <div className="absolute -bottom-2 -right-2 bg-orange-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white">
                                {progress}%
                              </div>
                            )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {progress > 0 && (
                              <div className="mt-3 max-w-[200px]">
                                <ProgressBar value={progress} max={100} colorClass={progress === 100 ? "bg-green-500" : "bg-orange-500"} />
                              </div>
                            )}
                        </div>
                                      ...
                                      {item.modules.map((mod, mIdx) => (
                                          <div key={mIdx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                              <div className="font-bold text-slate-900 mb-2 pb-2 border-b border-slate-100 flex items-center gap-2">
                                                  <Book className="w-4 h-4 text-orange-500" /> {mod.name}
                                              </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
                                        ...
                                        {item.practice_volume.case_studies && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Case Studies</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.case_studies} tình huống</span>
                                                </div>
                                        ...
                                        {item.practice_volume.review_sessions && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Phiên ôn tập</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.review_sessions} buổi</span>
                                                </div>
                                                <ProgressBar value={progress} max={100} colorClass="bg-green-500" />
                                            </div>
                                        )}
                                    </div>
                                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-gradient-to-r from-red-600 to-rose-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <Gavel className="w-5 h-5 text-red-200" /> {data.violation_types.title}
                    </h3>
                </div>

                {/* Violation Cards */}
                <div className="p-4 space-y-3 max-h-[300px] overflow-y-auto bg-slate-50/50">
                    {data.violation_types.types.map((type, idx) => (
                        <div key={idx} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                            <div className="font-bold text-red-700 text-sm mb-1">{type.name}</div>
                            ...
                            <div className="text-xs text-slate-500">
                                <span className="font-medium">VD:</span> {type.examples.join(', ')}
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <Users className="w-5 h-5 text-blue-200" /> {data.citizen_rights.title}
                    </h3>
                </div>
                
                {/* Tabs */}
                <div className="flex overflow-x-auto border-b border-slate-100 p-2 gap-1 no-scrollbar">
                    {data.citizen_rights.categories.map((cat, idx) => (
                        <button
                            key={idx}
                            onClick={() => setActiveRightsTab(idx)}
                ...
                <div className="p-4 bg-slate-50/50">
                    <ul className="space-y-2">
                        {data.citizen_rights.categories[activeRightsTab]?.rights.map((right, idx) => (
                            <li key={idx} className="bg-white p-3 rounded-lg border border-slate-200 text-sm shadow-sm flex items-start gap-2">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-emerald-200" /> {data.comparison_table.title}
                    </h3>
                </div>
                ...
                <div className="p-4 overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                        <thead>
                            <tr>
                                {data.comparison_table.headers.map((header, idx) => (
                        ...
                        <tbody>
                            {data.comparison_table.rows.map((row, rIdx) => (
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-100 p-6">
                <h3 className="font-bold text-yellow-800 mb-4 flex items-center gap-2">
                    <Lightbulb className="w-5 h-5" /> Mẹo Ôn Thi GDCD
                </h3>
            ...
            <div className="bg-orange-50 rounded-2xl border border-orange-100 p-6">
                <h3 className="font-bold text-orange-800 mb-4 flex items-center gap-2">
                    <Scale className="w-5 h-5" /> Cấu Trúc Đề Thi
                </h3>
                <div className="space-y-3">
                    {data.exam.structure.map((part, idx) => (
                        <div key={idx} className="bg-white/60 p-3 rounded-lg">
                            <div className="flex justify-between items-center mb-2">
                                <div className="font-bold text-orange-700 text-sm">{part.part}</div>
                                <div className="bg-orange-200 text-orange-700 px-2 py-1 rounded-full text-xs font-bold">
