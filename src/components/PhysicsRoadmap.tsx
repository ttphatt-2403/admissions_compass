...
import { physicsRoadmap } from '../roadmap/physics-roadmap';
...
const data = physicsRoadmap as PhysicsRoadmapData;
...
export function PhysicsRoadmap() {
  ...
  const [activeFormulaTab, setActiveFormulaTab] = useState<string>('dao_dong_co');
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('physics-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('physics-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-gradient-to-r from-amber-600 to-orange-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <Calculator className="w-5 h-5 text-yellow-300" /> Sổ Tay Công Thức
                    </h3>
                </div>
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-100 p-6">
                <h3 className="font-bold text-yellow-800 mb-4 flex items-center gap-2">
                    <Lightbulb className="w-5 h-5" /> Mẹo Ôn Thi Vật Lý
                </h3>
