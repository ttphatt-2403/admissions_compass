...
import { biologyRoadmap } from '../roadmap/biology-roadmap';
...
interface PracticeVolume {
  ...
  mini_tests?: number;
  full_tests?: number;
  full_tests_genetics?: number;
  calculation_exercises?: number;
  genetics_problems?: number;
...
const data = biologyRoadmap as BiologyRoadmapData;

// --- Components ---

const StatusBadge = ({ phase }: { phase: string }) => {
  const getColor = (p: string) => {
    if (p.includes('Foundation')) return 'bg-rose-100 text-rose-700 border-rose-200';
...
const ChecklistItem = ({ 
  ...
  return (
    <div 
      ...
      onClick={() => onToggle(id)}
    >
      <div className={`mt-0.5 shrink-0 transition-colors ${checked ? 'text-rose-600' : 'text-slate-300 group-hover:text-rose-400'}`}>
...
export function BiologyRoadmap() {
  const [expandedMonth, setExpandedMonth] = useState<string | null>(data.timeline[0].month);
  const [activeFormulaTab, setActiveFormulaTab] = useState<string>('di_truyen_phan_tu');
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('biology-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('biology-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
  ...
  const formulaCategories = {
    di_truyen_phan_tu: "Di truyền phân tử",
    di_truyen_menden: "Di truyền Menđen",
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-slate-500 text-sm font-medium mb-1">
                ...
                <span className="text-rose-600 font-bold">Môn {data.subject}</span>
              </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between mb-2">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-rose-600" /> Timeline Chi Tiết
                </h2>
                ...
                return (
                  <div 
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                        isExpanded 
                        ? 'border-rose-500 ring-4 ring-rose-500/10 shadow-lg relative z-10' 
                        : 'border-slate-200 hover:border-rose-200 hover:shadow-md'
                    } ${isComplete ? 'bg-rose-50/30' : ''}`}
                  >
                    {/* Header Card */}
                    <div 
                    ...
                    >
                        ...
                        <div className={`relative flex flex-col items-center justify-center w-16 h-16 rounded-xl border shrink-0 transition-colors ${
                            ...
                            {!isComplete && progress > 0 && (
                              <div className="absolute -bottom-2 -right-2 bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white">
                                {progress}%
                              </div>
                            )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {progress > 0 && (
                              <div className="mt-3 max-w-[200px]">
                                <ProgressBar value={progress} max={100} colorClass={progress === 100 ? "bg-green-500" : "bg-rose-500"} />
                              </div>
                            )}
                        </div>
                                      ...
                                      {item.modules.map((mod, mIdx) => (
                                          <div key={mIdx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                              <div className="font-bold text-slate-900 mb-2 pb-2 border-b border-slate-100 flex items-center gap-2">
                                                  <Book className="w-4 h-4 text-rose-500" /> {mod.name}
                                              </div>
                                    ...
                                    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
                                        {item.practice_volume.mcq_questions && (
                                            <div>
                                                ...
                                                <ProgressBar value={progress} max={100} colorClass="bg-rose-500" />
                                            </div>
                                        )}
                                        ...
                                        {item.practice_volume.full_tests_genetics && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Đề di truyền</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.full_tests_genetics} đề</span>
                                                </div>
                                                <ProgressBar value={progress} max={100} colorClass="bg-pink-500" />
                                            </div>
                                        )}
                                        {item.practice_volume.calculation_exercises && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Bài tập tính toán</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.calculation_exercises} bài</span>
                                                </div>
                                                <ProgressBar value={progress} max={100} colorClass="bg-blue-500" />
                                            </div>
                                        )}
                                        {item.practice_volume.genetics_problems && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    <span className="text-slate-600">Bài toán di truyền</span>
                                                    <span className="font-bold text-slate-900">{item.practice_volume.genetics_problems} bài</span>
                                                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-gradient-to-r from-rose-600 to-pink-600 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-yellow-300" /> Sổ Tay Công Thức
                    </h3>
                </div>
                    ...
                    {Object.keys(formulaCategories).map((key) => (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${
                                activeFormulaTab === key 
                                ? 'bg-rose-100 text-rose-700' 
                                : 'bg-transparent text-slate-500 hover:bg-slate-50'
                            }`}
                        >
                            {/* @ts-ignore */}
                            {formulaCategories[key]}
                        </button>
                    ))}
                </div>
                ...
                <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
                    <button className="text-xs font-bold text-rose-600 hover:underline flex items-center justify-center gap-1 w-full">
                        Xem toàn bộ công thức <ArrowRight className="w-3 h-3" />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-100 p-6">
                <h3 className="font-bold text-yellow-800 mb-4 flex items-center gap-2">
                    <Lightbulb className="w-5 h-5" /> Mẹo Ôn Thi Sinh Học
                </h3>
            ...
            {data.exam.structure && (
              <div className="bg-rose-50 rounded-2xl border border-rose-100 p-6">
                  <h3 className="font-bold text-rose-800 mb-4 flex items-center gap-2">
                      <Dna className="w-5 h-5" /> Cấu Trúc Đề Thi
                  </h3>
                  <div className="space-y-3">
                      {data.exam.structure.map((part, idx) => (
                          <div key={idx} className="bg-white/60 p-3 rounded-lg flex justify-between items-center">
                              <div>
                                  <div className="font-bold text-rose-700 text-sm">{part.part}</div>
                                  <div className="text-xs text-rose-500">{part.chapter}</div>
                              </div>
                              <div className="bg-rose-200 text-rose-700 px-2 py-1 rounded-full text-xs font-bold">
                                  {part.questions} câu
                              </div>
                          </div>
                      ))}
