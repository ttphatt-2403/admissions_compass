...
import { technologyRoadmap } from '../roadmap/technology-roadmap';
...
interface TechnologyRoadmapData {
    subject: string;
    exam: {
...
const data = technologyRoadmap as unknown as TechnologyRoadmapData;
...
export function TechnologyRoadmap() {
    ...
    const [activeFormulaTab, setActiveFormulaTab] = useState<string>('A_nen_tang_an_toan');
    const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

    // Load state from localStorage on mount
    useEffect(() => {
        const saved = localStorage.getItem('technology-roadmap-progress');
        if (saved) {
            try {
    ...
    useEffect(() => {
        localStorage.setItem('technology-roadmap-progress', JSON.stringify(checkedItems));
    }, [checkedItems]);
    ...
    const formulaCategories = {
        A_nen_tang_an_toan: "Nền tảng & An toàn",
        B_thiet_ke_vat_lieu: "Thiết kế & Vật liệu",
        C_kinh_doanh_san_xuat: "Kinh doanh & Sản xuất",
        D_ung_dung_cong_nghe: "Ứng dụng Công nghệ"
    };
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            <div className="flex items-center gap-2 text-slate-500 text-sm font-medium mb-1">
                                ...
                                <span className="text-blue-600 font-bold">Môn {data.subject}</span>
                            </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <div className="flex items-center justify-between mb-2">
                            <h2 className="text-xl font-bold flex items-center gap-2">
                                <Calendar className="w-5 h-5 text-blue-600" /> Timeline Chi Tiết
                            </h2>
                                ...
                                return (
                                    <div
                                        ...
                                        className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${isExpanded
                                            ? 'border-blue-500 ring-4 ring-blue-500/10 shadow-lg relative z-10'
                                            : 'border-slate-200 hover:border-blue-200 hover:shadow-md'
                                            } ${isComplete ? 'bg-blue-50/30' : ''}`}
                                    >
                                        {/* Header Card */}
                                        <div
                                        ...
                                        >
                                            ...
                                            <div className={`relative flex flex-col items-center justify-center w-16 h-16 rounded-xl border shrink-0 transition-colors ${isComplete
                                                ...
                                                {!isComplete && progress > 0 && (
                                                    <div className="absolute -bottom-2 -right-2 bg-blue-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white">
                                                        {progress}%
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {progress > 0 && (
                                                    <div className="mt-3 max-w-[200px]">
                                                        <ProgressBar value={progress} max={100} colorClass={progress === 100 ? "bg-green-500" : "bg-blue-500"} />
                                                    </div>
                                                )}
                                            </div>
                                                            ...
                                                            {item.modules.map((mod, mIdx) => (
                                                                <div key={mIdx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                                                    <div className="font-bold text-slate-900 mb-2 pb-2 border-b border-slate-100 flex items-center gap-2">
                                                                        <Book className="w-4 h-4 text-blue-500" /> {mod.name}
                                                                    </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                            <div className="p-4 bg-slate-900 text-white flex items-center justify-between">
                                <h3 className="font-bold flex items-center gap-2">
                                    <Cpu className="w-5 h-5 text-yellow-400" /> Sổ Tay Kiến Thức
                                </h3>
                            </div>
                            ...
                            <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
                                <button className="text-xs font-bold text-blue-600 hover:underline flex items-center justify-center gap-1 w-full">
                            ...
                            <div className="bg-blue-50 rounded-2xl border border-blue-100 p-6">
                                ...
                                <div className="space-y-3">
                                    {data.exam.structure.map((part, idx) => (
                                        <div key={idx} className="bg-white/60 p-3 rounded-lg flex justify-between items-center shadow-sm border border-blue-100">
                                            <div>
                                                <div className="font-bold text-blue-700 text-sm">{part.part}</div>
                                                <div className="text-xs text-blue-500 font-medium">{part.chapter}</div>
                                            </div>
                                            <div className="bg-blue-200/50 text-blue-700 px-2 py-1 rounded-full text-xs font-bold shrink-0">
                                                {part.questions} câu
                                            </div>
                                        </div>
                                    ))}
