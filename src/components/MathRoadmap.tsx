...
import { mathRoadmap } from '../roadmap/math-roadmap';
...
const data = mathRoadmap as MathRoadmapData;
...
export function MathRoadmap() {
  const [expandedMonth, setExpandedMonth] = useState<string | null>(data.timeline[0].month);
  const [activeFormulaTab, setActiveFormulaTab] = useState<string>('ham_so_dao_ham');
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // Load state from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('math-roadmap-progress');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('math-roadmap-progress', JSON.stringify(checkedItems));
  }, [checkedItems]);
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 font-medium">
                                ...
                                <span>{item.modules.length} chủ đề</span>
                            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                <div className="p-4 bg-slate-900 text-white flex items-center justify-between">
                    <h3 className="font-bold flex items-center gap-2">
                        <Calculator className="w-5 h-5 text-yellow-400" /> Sổ Tay Công Thức
                    </h3>
                </div>
                ...
                <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
                    <button className="text-xs font-bold text-blue-600 hover:underline flex items-center justify-center gap-1 w-full">
                        Xem toàn bộ 100+ công thức <ArrowRight className="w-3 h-3" />
                    </button>
                </div>
            </div>
